...
parser.add_argument('--vote_factor',
                    type=int,
                    default=1,
...
parser.add_argument('--overwrite',
                    action='store_true',
                    help='Overwrite existing log and dump folders.')
parser.add_argument('--dump_results',
                    action='store_true',
                    help='Dump results.')
...
parser.add_argument('--print_interval',
                    type=int,
                    default=10,
                    help='batch inverval to print loss')
parser.add_argument('--eval_interval',
                    type=int,
                    default=50,
                    help='epoch inverval to evaluate model')
...
if os.path.exists(LOG_DIR) and FLAGS.overwrite:
    print('Log folder %s already exists. Are you sure to overwrite? (Y/N)' %
          (LOG_DIR))
    c = input()
    if c == 'n' or c == 'N':
        print('Exiting..')
...
if FLAGS.dataset == 'sunrgbd':
    ...
    TEST_DATASET = SunrgbdDetectionVotesDataset(
        ...
        use_height=(not FLAGS.no_height),
        use_v1=(not FLAGS.use_sunrgbd_v2)
    )  #, labeled_sample_list='sun_vis.txt')# single=FLAGS.vis_single)
    ...
    TEST_DATASET = ScannetDetectionDataset(
        ...
        use_color=FLAGS.use_color,
        use_height=(
            not FLAGS.no_height))  #, labeled_sample_list='scan_vis.txt')#
else:
