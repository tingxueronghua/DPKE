...
def decode_scores(net, end_points, num_class, num_heading_bin,
    ...
    center = base_xyz + net_transposed[:, :,
                                       2:5]  # (batch_size, num_proposal, 3)
    end_points['center'] = center

    heading_scores = net_transposed[:, :, 5:5 + num_heading_bin]
    heading_residuals_normalized = net_transposed[:, :, 5 + num_heading_bin:5 +
                                                  num_heading_bin * 2]
    end_points[
        'heading_scores'] = heading_scores  # Bxnum_proposalxnum_heading_bin
    end_points[
    ...
    size_scores = net_transposed[:, :, 5 + num_heading_bin * 2:5 +
                                 num_heading_bin * 2 + num_size_cluster]
    ...
    sem_cls_scores = net_transposed[:, :, 5 + num_heading_bin * 2 +
                                    num_size_cluster * 4:]  # Bxnum_proposalx10
...
class ProposalModule(nn.Module):
    ...
    def forward(self, xyz, features, end_points, vote_inds=None):
        """
        Args:
            xyz: (B,K,3)
            features: (B,C,K)
        ...
        if self.sampling == 'vote_fps':
            raise ValueError('do not expect this fps')
        ...
        end_points['aggregated_vote_features'] = features.transpose(1, 2)
        # --------- PROPOSAL GENERATION ---------
        net = F.relu(self.bn1(self.conv1(features)))
        end_points['conv1_features'] = net.transpose(1, 2)
        net = F.relu(self.bn2(self.conv2(net)))
        end_points['conv2_features'] = net.transpose(1, 2)
        net = self.conv3(
            net
