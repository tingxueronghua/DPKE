...
class GridConv(nn.Module):
    ...
    def forward(self, center, size, heading, end_points):
        ...
        grid_size = 4
        grid_step_x = grid_step.view(grid_size, 1,
                                     1).repeat(1, grid_size, grid_size)
        grid_step_x = grid_step_x.view(1, 1, -1).expand(B, K, -1)
        grid_step_y = grid_step.view(1, grid_size,
                                     1).repeat(grid_size, 1, grid_size)
        grid_step_y = grid_step_y.view(1, 1, -1).expand(B, K, -1)
        grid_step_z = grid_step.view(1, 1,
                                     grid_size).repeat(grid_size, grid_size, 1)
        ...
        whole_grid = torch.bmm(whole_grid.view(B * K, -1, 3),
                               rot_mat.transpose(1, 2)).view(B, K, -1, 3)
        whole_grid = whole_grid + center_xyz.unsqueeze(2).expand(
            -1, -1, grid_size * grid_size * grid_size, -1)
        ...
        interp_points = torch.gather(origin_xyz,
                                     dim=1,
                                     index=idx.view(B, -1, 1).expand(
                                         -1, -1, 3).long())  # B, K*64*3, 3
        expanded_whole_grid = whole_grid.unsqueeze(2).expand(
            -1, -1, 3, -1).contiguous().view(B, -1, 3)  # B, K*64*3, 3
        ...
        relative_grid = whole_grid - center_xyz.unsqueeze(2).expand(
            -1, -1, grid_point_num, -1).contiguous().view(B, -1, 3)
        ...
        interpolated_feats = torch.cat([
            ...
            for a, i in zip(origin_features.transpose(1, 2),
                            idx.view(B, -1).long())
        ], 0)
        interpolated_feats = torch.sum(
            interpolated_feats.view(B, -1, 3, feat_size) *
            weight.unsqueeze(-1),
            dim=2)
        interpolated_feats = interpolated_feats.transpose(1, 2)
        interpolated_feats = interpolated_feats.view(B, -1, K, grid_point_num)
        interpolated_feats = torch.cat([
            relative_grid.transpose(1, 2).contiguous().view(B, -1, K, 64),
            interpolated_feats
        ],
                                       dim=1)
        interpolated_feats = self.mlp_before_iou(
            interpolated_feats)  # B, C, K, 64

        iou_features = F.max_pool2d(interpolated_feats,
                                    kernel_size=[
                                        1, interpolated_feats.size(3)
                                    ]).squeeze(-1)
        ...
        end_points['iou_scores'] = net_iou.transpose(2, 1)[:, :,
                                                           -self.iou_size:]
        return end_points
