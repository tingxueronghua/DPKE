...
class VoteNet(nn.Module):
    ...
    def __init__(
        ...
        self.pnet = ProposalModule(num_class,
                                   num_heading_bin,
                                   num_size_cluster,
                                   mean_size_arr,
                                   num_proposal,
    ...
    def forward_backbone(self,
        ...
        end_points = self.pnet(xyz, features, end_points, vote_inds=vote_inds)
        return end_points
    ...
    def calculate_bbox(self, end_points):
        ...
        size_residual = torch.gather(
            size_residuals, 2,
            size_class.unsqueeze(-1).unsqueeze(-1).expand(
        ...
        heading_residual = torch.gather(
            heading_residuals, 2,
            heading_class.unsqueeze(-1))  # B,num_proposal,1
        heading_residual = heading_residual.squeeze(2)
        heading = self.dataset_config.class2angle_gpu(heading_class,
                                                      heading_residual)
    ...
    def forward(self,
        ...
        if iou_opt:
            ...
            end_points = self.grid_conv(center.detach(), size.detach(),
                                        heading.detach(), end_points)
        return end_points
    ...
    def forward_with_pred_jitter(self,
                                 inputs,
                                 sa1_inds=None,
                                 sa2_inds=None,
                                 sa3_inds=None,
        ...
        factor = 1
        center_jitter = center.unsqueeze(2).expand(-1, -1, factor,
                                                   -1).contiguous().view(
                                                       B, -1, 3)
        size_jitter = size.unsqueeze(2).expand(-1, -1, factor,
                                               -1).contiguous().view(B, -1, 3)
        heading_jitter = heading.unsqueeze(2).expand(-1, -1,
                                                     factor).contiguous().view(
                                                         B, -1)
        center_jitter = center_jitter + size_jitter * torch.randn(
            size_jitter.shape).cuda() * 0.3
        size_jitter = size_jitter + size_jitter * torch.randn(
            size_jitter.shape).cuda() * 0.3
        ...
        end_points = self.grid_conv(center.detach(), size.detach(),
                                    heading.detach(), end_points)
        end_points['iou_scores_jitter'] = end_points[
            'iou_scores'][:, origin_proposal_num:]
        end_points['iou_scores'] = end_points[
            'iou_scores'][:, :origin_proposal_num]
