...
def trans_center(center, flip_x_axis, flip_y_axis, rot_mat, scale_ratio):
    ...
    center_clone[inds_to_flip_x_axis, :,
                 0] = -center[inds_to_flip_x_axis, :, 0]

    inds_to_flip_y_axis = torch.nonzero(flip_y_axis).squeeze(1)
    center_clone[inds_to_flip_y_axis, :,
                 1] = -center[inds_to_flip_y_axis, :, 1]

    center_clone = torch.bmm(center_clone,
                             rot_mat.transpose(1, 2))  # (B, num_proposal, 3)
...
def trans_angle(angle_class, angle_residual, flip_x_axis, flip_y_axis,
                rot_angle, config):
...
def reverse_trans_center(center, flip_x_axis, flip_y_axis, rot_mat,
                         scale_ratio):
...
def compute_objectness_gt(end_points, unsupervised_inds):
    ...
    gt_mask = (1 - end_points['box_label_mask'][unsupervised_inds,
                                                ...]).unsqueeze(-1).expand(
                                                    -1, -1, 3).bool()
    ...
    objectness_label = torch.zeros((batch_size, num_proposal),
    ...
    obj_pred_val = torch.argmax(
        end_points['objectness_scores'][unsupervised_inds, ...], 2)  # B,K
    obj_acc = torch.sum((obj_pred_val == objectness_label.long()).float() *
                        objectness_mask) / mask_sum
...
def compute_objectness_loss(end_points, unsupervised_inds, config_dict):
    ...
    # ---- mod -----
    gt_mask = (1 -
               end_points['unlabeled_box_label_mask']).unsqueeze(-1).expand(
                   -1, -1, 3).bool()
    gt_center[gt_mask] = -1000
    # ----------------
    ...
    if config_dict['samecls_match']:
        dist1, ind1, dist2, ind2 = nn_distance_withcls(
            aggregated_vote_xyz, gt_center,
            torch.argmax(end_points['sem_cls_scores'][unsupervised_inds, ...],
                         dim=2), end_points['unlabeled_sem_cls_label'])
    else:
...
def compute_box_and_sem_cls_loss(end_points, unsupervised_inds, config,
    ...
    # Compute center loss
    dist1, ind1, dist2, _ = nn_distance(
        end_points['center'][unsupervised_inds, ...],
        end_points['unlabeled_center_label'][:, :,
    ...
    heading_class_label = torch.gather(
        end_points['unlabeled_heading_class_label'], 1,
        object_assignment)  # select (B,K) from (B,K2)
    criterion_heading_class = nn.CrossEntropyLoss(reduction='none')
    heading_class_loss = criterion_heading_class(
        end_points['heading_scores'][unsupervised_inds, ...].transpose(2, 1),
    ...
    heading_residual_label = torch.gather(
        end_points['unlabeled_heading_residual_label'], 1,
        object_assignment)  # select (B,K) from (B,K2)
    ...
    heading_residual_pred = torch.sum(
        end_points['heading_residuals_normalized'][unsupervised_inds, ...] *
        heading_label_one_hot, -1)
    heading_diff = heading_residual_pred - heading_residual_normalized_label
    heading_residual_normalized_loss = huber_loss(heading_diff,
                                                  delta=1.0)  # (B,K)
    ...
    size_class_label = torch.gather(
        end_points['unlabeled_size_class_label'], 1,
        object_assignment)  # select (B,K) from (B,K2)
    criterion_size_class = nn.CrossEntropyLoss(reduction='none')
    size_class_loss = criterion_size_class(
        end_points['size_scores'][unsupervised_inds, ...].transpose(2, 1),
    ...
    predicted_size_residual_normalized = torch.sum(
        end_points['size_residuals_normalized'][unsupervised_inds, ...] *
    ...
    size_residual_normalized_loss = torch.mean(huber_loss(size_diff,
                                                          delta=1.0),
                                               -1)  # (B,K,3) -> (B,K)
    ...
    sem_cls_loss = criterion_sem_cls(
        end_points['sem_cls_scores'][unsupervised_inds, ...].transpose(2, 1),
        sem_cls_label)  # (B,K)
...
def get_pseudo_detection_loss(end_points, ema_end_points, config, config_dict):
    ...
    total_num_proposal = pseudo_objectness_label.shape[
        0] * pseudo_objectness_label.shape[1]
    ...
    if config_dict['view_stats']:
        ...
        obj_acc = torch.sum(
            (obj_pred_val == gt_objectness_label.long()).float() *
            gt_objectness_mask) / (torch.sum(gt_objectness_mask) + 1e-6)
...
def get_pseudo_labels(end_points, ema_end_points, pred_center, pred_sem_cls,
    ...
    original_inds = torch.argsort(pos_obj * max_cls * final_mask,
                                  dim=1,
                                  descending=True)

    inds = original_inds[:, :MAX_NUM_OBJ].long()
    ...
    unlabeled_inds = original_inds

    return label_mask, center_label, sem_cls_label, heading_label, heading_residual_label, size_label, size_residual_label, false_center_label, iou_label, unlabeled_inds


def get_unlabeled_loss(end_points, ema_end_points, config, config_dict):
    unlabeled_batch_size = config_dict['unlabeled_batch_size']
    labeled_num = torch.nonzero(
        end_points['supervised_mask']).squeeze(1).shape[0]
    ...
    false_center_label, iou_label, unlabeled_inds = \
    ...
    center_label = trans_center(center_label,
    ...
    false_center_label = trans_center(false_center_label,
                                      end_points['flip_x_axis'][labeled_num:],
                                      end_points['flip_y_axis'][labeled_num:],
                                      end_points['rot_mat'][labeled_num:],
                                      end_points['scale'][labeled_num:])
    size_residual_label = trans_size(size_label, size_residual_label,
                                     end_points['scale'][labeled_num:], config)
    if config_dict['dataset'] == 'sunrgbd':
        heading_label, heading_residual_label = trans_angle(
            heading_label, heading_residual_label,
            end_points['flip_x_axis'][labeled_num:],
            end_points['flip_y_axis'][labeled_num:],
    ...
    if config_dict['view_stats']:
        ...
        end_points['center_label'][labeled_num:] = trans_center(
            end_points['center_label'][labeled_num:],
            end_points['flip_x_axis'][labeled_num:],
            end_points['flip_y_axis'][labeled_num:],
            end_points['rot_mat'][labeled_num:],
            end_points['scale'][labeled_num:])

        end_points['size_residual_label'][labeled_num:] = trans_size(
            end_points['size_class_label'][labeled_num:],
    ...
    end_points['unlabeled_inds'] = unlabeled_inds

    consistency_loss, end_points = get_pseudo_detection_loss(
