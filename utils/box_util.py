...
def boxes3d_iou_batch(batch_corners1, batch_corners2):
    ...
    for i in range(m):
        rect2 = [(batch_corners2[i, k, 0], batch_corners2[i, k, 2])
                 for k in range(3, -1, -1)
                 ]  #[((0,0),(0,2)),..., ((3,0),(3,2))]
        vol2 = vol_batch2[i]

        y_max = np.where(y_max_batch1 - y_max_batch2[i] < 0, y_max_batch1,
                         y_max_batch2[i])  #n
        y_min = np.where(y_min_batch1 - y_min_batch2[i] > 0, y_min_batch1,
                         y_min_batch2[i])  #n
        inter_y = np.where(y_max - y_min < 0., 0., y_max - y_min)  #n
        inter_area = np.zeros((n), dtype=np.float32)  #n
        for j in range(n):
            rect1 = [(batch_corners1[j, k, 0], batch_corners1[j, k, 2])
                     for k in range(3, -1, -1)]
...
def box3d_iou_batch(corners1, corners2):
    ...
    diff = np.max(np.stack(
        [min_max - max_min, np.zeros(min_max.shape)], axis=2),
                  axis=2)
...
def box3d_iou_gpu_axis_aligned(corners1, corners2):
    ...
    diff = torch.max(torch.stack(
        [min_max - max_min,
         torch.zeros(min_max.shape).cuda()], dim=2),
                     dim=2)[0]
...
def check_valid_corners3d(corners_3d):
    ...
    '''
    x_lines = np.vstack(
        (corners_3d[0] - corners_3d[3], corners_3d[1] - corners_3d[2],
         corners_3d[4] - corners_3d[7], corners_3d[5] - corners_3d[6]))  #(4,3)
    y_lines = np.vstack(
        (corners_3d[0] - corners_3d[4], corners_3d[1] - corners_3d[5],
         corners_3d[3] - corners_3d[7], corners_3d[2] - corners_3d[6]))  #(4,3)
    z_lines = np.vstack(
        (corners_3d[0] - corners_3d[1], corners_3d[4] - corners_3d[5],
    ...
    lines_length = np.vstack(
        (x_lines_length, y_lines_length, z_lines_length)).transpose()  #(4,3)

    try:
        npt.assert_almost_equal(lines_length[0],
                                np.array([0., 0., 0.]),
                                decimal=1)
        print('\t\tWarning: this length of box [{0}] is almost zero...'.format(
            lines_length[0]))
        return False
    except:
        pass
    ...
    try:
        ...
        npt.assert_almost_equal(
            (corners_3d[0] - corners_3d[4]) @ (corners_3d[0] - corners_3d[1]),
            0,
            decimal=1)
        npt.assert_almost_equal(
            (corners_3d[0] - corners_3d[4]) @ (corners_3d[0] - corners_3d[3]),
            0,
            decimal=1)
        npt.assert_almost_equal(
            (corners_3d[0] - corners_3d[1]) @ (corners_3d[0] - corners_3d[3]),
            0,
            decimal=1)
