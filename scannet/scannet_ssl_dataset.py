...
from utils import scannet_sampling
import ipdb
...
class ScannetSSLLabeledDataset(Dataset):
    def __init__(self,
        ...
        split_filenames = os.path.join(BASE_DIR,
                                       'meta_data/scannetv2_train.txt')
        with open(split_filenames, 'r') as f:
            train_scan_names = f.read().splitlines()
        # remove unavailiable scans
        train_scan_names = [
            sname for sname in train_scan_names if sname in all_scan_names
        ]
        ...
        if labeled_sample_list is not None:
            ...
            if len(train_scan_names) == len(self.scan_names):
                self.unlabelled_scan_names = self.scan_names
            else:
                self.unlabelled_scan_names = list(
                    set(train_scan_names) - set(self.scan_names))
        else:
        ...
        base_categories = self.gt_base['instance_bboxes'][:, -1]
        self.base_class_dict = {}
        for i, indx in enumerate(DC.nyu40ids):
            self.base_class_dict[i] = np.where(base_categories == indx)[0]
    ...
    def __getitem__(self, idx):
        ...
        if self.prob_aug:
            ...
            augmented_dict = scannet_sampling.sample_from_base(
                input_dict,
                self.gt_base,
                self.base_class_dict,
                category_list=self.category_list,
                category_prob=self.category_prob)
...
class ScannetSSLUnlabeledDataset(Dataset):
    def __init__(self,
                 ...
                 prob_aug=True):
        print(
            '----------------Scannet Unlabeled Dataset Initialization----------------'
        )
        self.data_path = os.path.join(BASE_DIR, 'scannet_train_detection_data')
        all_scan_names = list(set([os.path.basename(x)[0:12] \
                                   for x in os.listdir(self.data_path) if x.startswith('scene')]))
        ...
        if labeled_sample_list is not None:
            ...
            if len(train_scan_names) == len(labeled_scan_names):
                ...
                self.scan_names = list(
                    set(train_scan_names) - set(labeled_scan_names))
            print('\tGet {} unlabeled scans out of {}'.format(
                len(self.scan_names), len(train_scan_names)))
        else:
            print('\tUnknown labeled sample list: %s. Exiting...' %
                  labeled_sample_list)
            exit(-1)
    ...
    def __getitem__(self, idx):
        ...
        if self.prob_aug:
            ...
            augmented_dict = scannet_sampling.sample_from_base(
                input_dict, self.gt_base, self.base_class_dict)
            mesh_vertices = augmented_dict['point_clouds']
            instance_bboxes = augmented_dict['bboxes']
