...
# different from previous versions, because want to disentangle dataset and py files.
# BASE_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = ""  # path to your scannet dataset
...
class ScannetDetectionDataset(Dataset):
    def __init__(self,
        ...
        if split_set == 'all':
            ...
            split_filenames = os.path.join(
                BASE_DIR, 'meta_data', 'scannetv2_{}.txt'.format(split_set))
        ...
        # added
        self.raw_data_path = os.path.join(BASE_DIR, 'meta_data')
        ...
        if split_set == 'train':
            ...
            gt_base_filename = os.path.join(
                BASE_DIR, '{}_gt_base_no_instance_segmentation.pkl'.format(
                    labeled_sample_list))
            with open(gt_base_filename, 'rb') as f:
                self.gt_base = pkl.load(f)

    def sample_from_base(self, input_dict, base, use_mask=True):
        '''
        ...
        if len(indexes_final) == 0:
            ...
            indexes_final = np.random.choice(indexes_final, (self.sample_num),
                                             replace=False)
        ...
        for indx in indexes_final:
            res_point_clouds = np.concatenate(
                [res_point_clouds, base_raw_pc[indx]], axis=0)
            res_bboxes = np.concatenate(
                [res_bboxes, base_bboxes[indx][np.newaxis, ...]], axis=0)
            tem_array = np.ones(base_raw_pc[indx].shape[0])
            res_instance_labels = np.concatenate([
                res_instance_labels,
                tem_array * np.max(res_instance_labels + 1)
            ])
            res_semantic_labels = np.concatenate(
                [res_semantic_labels, tem_array * base_bboxes[indx, -1]])
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and self.split_set == 'train':
            ...
            augmented_dict = self.sample_from_base(input_dict, self.gt_base)
            mesh_vertices = augmented_dict['point_clouds']
            instance_labels = augmented_dict['instance_labels']
            semantic_labels = augmented_dict['semantic_labels']
            instance_bboxes = augmented_dict['bboxes']

        if self.remove_obj:
            if np.random.random() > 0.5 and instance_bboxes.shape[0] >= 3:
                #random remove an object
                removed_box_ind = random.choice(
                    list(range(0, instance_bboxes.shape[0])))
                # NOTE: this assumes obj_id is in 1,2,3,.,,,.NUM_INSTANCES
                removed_obj_ind = removed_box_ind + 1
                removed_verts_inds = np.where(
                    instance_labels == removed_obj_ind)[0]

                instance_bboxes = np.delete(instance_bboxes,
                                            removed_box_ind,
                                            axis=0)
                mesh_vertices = np.delete(mesh_vertices,
                                          removed_verts_inds,
                                          axis=0)
                instance_labels = np.delete(instance_labels,
                                            removed_verts_inds,
                                            axis=0)
                semantic_labels = np.delete(semantic_labels,
                                            removed_verts_inds,
                                            axis=0)
    ...
    def get_labeled_samples(self):
        if self.labeled_sample_list is not None:
            labeled_scan_names = [
                x.strip() for x in open(
                    os.path.join(BASE_DIR, 'meta_data',
            ...
            for i, scan_name in enumerate(self.scan_names):
                instance_bboxes = np.load(
                    os.path.join(self.data_path, scan_name) + '_bbox.npy')
                class_ind = [
                    DC.nyu40id2class[x] for x in instance_bboxes[:, -1]
                ]
                if class_ind != []:
                    unique_class_ind = list(set(class_ind))
                else:
