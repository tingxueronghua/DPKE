...
from utils.sunrgbd_sampling import generate_votes
from utils import sunrgbd_sampling

# BASE_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = ""  # path of your sunrgbd dataset
...
class SunrgbdSSLLabeledDataset(Dataset):
    def __init__(self,
        ...
        if use_v1:
            self.data_path = os.path.join(
                BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v1_train')
        else:
            self.data_path = os.path.join(
                BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v2_train')

        if labeled_sample_list is not None:
            self.scan_names = [
                x.strip() for x in open(
                    os.path.join(BASE_DIR, 'sunrgbd_trainval',
        ...
        for indx in range(DC.num_class):
            self.base_class_dict[indx] = np.where(base_categories == indx)[0]
    ...
    def __getitem__(self, idx):
        ...
        if self.prob_aug:
            ...
            augmented_dict = sunrgbd_sampling.sample_from_base(
                input_dict, self.gt_base, self.base_class_dict)
            point_cloud = augmented_dict['point_clouds']
            bboxes = augmented_dict['bboxes']

        point_votes = generate_votes(point_cloud, bboxes, scan_name,
                                     point_cloud.shape[0])
        ...
        ema_point_cloud = pc_util.random_sampling(point_cloud,
                                                  self.num_points,
...
class SunrgbdSSLUnlabeledDataset(Dataset):
    def __init__(self,
                 ...
                 prob_aug=False):
        print(
            '----------------Sunrgbd Unlabeled Dataset Initialization----------------'
        )
        if use_v1:
            self.data_path = os.path.join(
                BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v1_train')
